...
class DispatchApprovalUserWizard(models.TransientModel):
    ...
    def _default_line_ids(self):
        ...
        res = [(0, 0, {
            ...
            'groups_id': node.get('groups_id'),
            'groups_id_domain': ','.join(map(lambda x: str(x), node.get('groups_ids') or list())),
            'user_id_domain': ','.join(map(lambda x: str(x), node.get('user_ids') or list()))})
    ...
    def button_ok(self):
        ...
        for path in paths:
            ...
            instance_node = instance_node_obj.create({
                ...
                # [('active', '草稿'), ('running', '正在审批'), ('complete', '完成')]
            })
            ...
            if node_type == 'free':
            ...
            elif node_type == 'group' or node_type == 'own_group':
                if user_id:
                    user_ids = [user_id]
                else:
                    if node_id.only_document_company:
                        user_ids = node_id.groups_id.users.filtered(
                            lambda user: user.company_id.id == document.company_id.id).ids
                    else:
                ...
                if user_id:
                    ...
                    if node_id.job_only_document_company:
                        employees = employee_obj.search(
                            [('company_id', '=', document.company_id.id), ('job_id', 'in', node_id.job_ids.ids),
                             ('user_id', '!=', False)])
                    else:
                        employees = employee_obj.search(
                ...
                if user_id:
                    ...
                    if node_id.user_only_document_company:
                        user_ids = node_id.user_ids.filtered(
                            lambda user: user.company_id.id == document.company_id.id).ids
                    else:
                ...
                if not user_id:
                    raise ValidationError(
                        '没有为员工%s的直属领导%s绑定用户，不能为节点%s指定直属领导审批类型！' % (employee.name, parent.name, node.name))
                ...
                if not user_id:
                    raise ValidationError(
                        '部门%s管理员%s绑定用户，不能为节点%s指定部门领导审批类型！' % (department.name, manager.name, node.name))
...
APPROVAL_TYPE = [('group', u'组'), ('own_group', u'本组'), ('free', '自由'), ('user', u'用户'), ('job', u'岗位'),
                 ('leader', u'直属领导'),
                 ('department_head', u'部门领导')]
...
class DispatchApprovalUserWizardLine(models.TransientModel):
    ...
    node_type = fields.Selection(APPROVAL_TYPE, '审批类型')

    groups_id = fields.Many2one('res.groups', '审批部门')
    user_id = fields.Many2one('res.users', '审批用户')
    user_id_domain = fields.Char('')
    groups_id_domain = fields.Char('')
