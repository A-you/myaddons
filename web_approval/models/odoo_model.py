...
def _compute_doc_approval_state(self):
    ...
    if res_state:
        ...
        if approval_state == 'draft' or not approval_state:
            ...
            wait_users = u','.join([wait_approval.user_id.sudo().name for wait_approval in self.env['wait.approval'].search([('model_name', '=', self._name), ('res_id', '=', self.id), ('state', '=', 'running')])])
            self.doc_approval_state = u'等待<span style="color:red">%s</span>审批' % wait_users
    else:
        self.doc_approval_state = u'未知状态'
