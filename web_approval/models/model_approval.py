...
def _commit_approval(self):
    ...
    def dispatch_approval_user():
        ...
        for path in paths:
            ...
            groups_id = node_id.groups_id  # 审批节点审批类型

            login_user_groups_ids = self.env.user.groups_id.ids
            # print("login_user_groups_ids开始：")
            # print(login_user_groups_ids)
            own_groups_ids = node_id.own_groups_ids.ids
            free_groups_ids = node_id.free_groups_ids.ids
            # free_groups_ids = node_id.groups_id
            # print("own_groups_ids开始：")
            # print(len(own_groups_ids))
            # print(own_groups_ids)
            if node_type == 'free':
                node_user.append({
                    'node_id': node.id,
                    'node_name': node.name,
                    'node_type': node_type,
                    'groups_ids': free_groups_ids,
                    'serial_num': path['serial_num'],
                })
            elif node_type == 'own_group':
                for group_id in login_user_groups_ids:
                    if group_id in own_groups_ids:
                        users = users_obj.search([('share', '=', False)])
                        exist_users = users.filtered(lambda user: group_id in user.groups_id.ids)
                ...
                if not node_id.is_all_approval:
                    ...
                    # exist_users2 = users.filtered(lambda user: node_id.groups_id.id in user.groups_id.ids)
                    if len(exist_users) > 1:
                        node_user.append({
                            'node_id': node.id,
                            'node_name': node.name,
    ...
    def recommit():
        ...
        instance = instance_obj.search(
            [('res_id', '=', res_id), ('model_name', '=', model), ('state', '=', 'complete'), ], order='id desc',
            limit=1)
    ...
    for path in paths:
        ...
        instance_node = instance_node_obj.create({
            ...
            # [('active', u'草稿'), ('running', u'正在审批'), ('complete', u'完成')]
        ...
        if node_type == 'group':
            ...
            if not user_id:
                raise ValidationError(
                    u'没有为员工%s的直属领导%s绑定用户，不能为节点%s指定直属领导审批类型！' % (employee.name, parent.name, node.name))
...
def _cancel_approval(self):
    ...
    # 删除消息
    self.env['mail.message'].sudo().search([('model', '=', model), ('res_id', '=', res_id), (
        'subtype_id', '=', self.env.ref('web_approval.mail_message_subtype_approval').id)]).unlink()
...
def _get_approval_info(self):
    ...
    def get_node_name():
        ...
        user_names = '，'.join(
            [wait_approval.user_id.name for wait_approval in wait_approvals if not wait_approval.is_turn_to])
    ...
    def get_swap(wa):
        ...
        for channel in channel_obj.search([('wait_approval_id', '=', wa.id)]):
            ...
            items = [{
                ...
                'date': Datetime.to_string(message.create_date.astimezone(pytz.timezone)),
                'content': message.body
            } for message in
                message_obj.search([('model', '=', 'mail.channel'), ('res_id', '=', channel.id)], order='id')]
            if not items:
                continue
    ...
    for instance in instance_obj.search([('model_name', '=', model), ('res_id', '=', res_id)], order='id desc'):
        ...
        for instance_node in instance_node_obj.sudo().search([('instance_id', '=', instance.id)],
                                                             order='serial_num desc, id desc'):

            wait_approvals = wait_approval_obj.search([('instance_node_id', '=', instance_node.id)],
                                                      order='serial_num asc, id desc')  # 实例节点对应的待审批
            wait_approvals_copy = []
            for wait_approval in wait_approvals:
                approval = approval_obj.search([('wait_approval_id', '=', wait_approval.id)])
                if approval:
                ...
                if wait_approval.is_turn_to:
                    if approval:
                        approvals.append({
                            ...
                            'approval': {  # 审批信息
                                ...
                                'approvalDate': Datetime.to_string(
                                    approval.create_date.replace(tzinfo=pytz.utc).astimezone(local_tz)),  #
                                'idea': approval.idea,  # 审批意见
                                'copyFor': '，'.join(
                                    [copy_for.copy_for_user_id.name for copy_for in approval.copy_for_ids]) or False,
                                # 抄送
                    ...
                    if approval:
                        approvals.append({
                            ...
                            'actionType': approval.action_type,
                            'nodeName': '%s转(%s)' % (
                                node_name, wait_approval.user_id.name) if wait_approval.is_turn_to else node_name,
                            'refuseNode': approval.refuse_node_id.name,  # 驳回节点
                            'turnToNode': False if not approval.turn_to_user_id else '转(%s)审批' % (
                                approval.turn_to_user_id.name,),  # 转签节点名称
                            'canTurnTo': False,  # 当前节点是否可转签
                            'allowIncrease': False,  # 允许加签
                            'approval': {  # 审批信息
                                'userName': approval.user_id.name,  # 审批用户名
            ...
            if unapproval_wait_approval:
                ...
                for wait_approval in unapproval_wait_approval:
                    if wait_approval.user_id.id == current_uid:
                        approvals.insert(0, {
                            ...
                            'allowIncrease': not wait_approval.is_increase and (
                                    instance_node.node_id.allow_before_increase or instance_node.node_id.allow_after_increase),
                            # 允许加签
                            'approval': {
                                'swap': get_swap(wait_approval)
                            },  # 审批信息
                ...
                if not exist:
                    ...
                    approvals.insert(0, {
                        ...
                        'turnToNode': False,  # 转签节点名称
                        'canTurnTo': instance_node.node_id.allow_turn_to,  # 当前节点是否可转签
                        'allowIncrease': instance_node.node_id.allow_before_increase or instance_node.node_id.allow_after_increase,
                        # 允许加签
                        'approval': {},  # 审批信息
                    })
        ...
        approval_info.append({
            ...
            'approval': {
                'userName': False,
                'approvalDate': False,
                'idea': '%s %s 提交' % (instance.create_uid.name if instance.create_uid.id != 1 else '系统',
                                      Datetime.to_string(
                                          instance.create_date.replace(tzinfo=pytz.utc).astimezone(local_tz))),
                'copyFor': False,
                'swap': []
            },
        })
