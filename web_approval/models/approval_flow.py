...
APPROVAL_TYPE = [('group', u'组'), ('own_group', u'本组'), ('free', '自由'), ('user', u'用户'), ('job', u'岗位'),
                 ('leader', u'直属领导'), ('department_head', u'部门领导')]
...
class ApprovalFlow(models.Model):
    ...
    approval_cannot_run = fields.Char('审批前运行', help='审批流程完成后不能能执行的功能，比如修改、删除等，用英文逗号间隔', track_visibility='onchange')
    ...
    def create(self, vals):
        ...
        return super(ApprovalFlow, self).create(vals)
...
class ApprovalNode(models.Model):
    ...
    # is_own_group = fields.Boolean(u'仅可指定本组', default=False, track_visibility='onchange')
    # 本组
    own_groups_ids = fields.Many2many('res.groups', string=u'参与审批用户组', track_visibility='onchange')

    # 本部门
    free_groups_ids = fields.Many2many('res.groups', string=u'参与审批用户组', track_visibility='onchange')
...
class Approval(models.Model):
    ...
    action_type = fields.Selection([('accept', u'同意'), ('refuse', u'拒绝'), ('turn_to', u'转签')], u'动作类型')
    ...
    turn_to_user_id = fields.Many2one('res.users', '转签用户ID')
