...
def _offset_format_timestamp1(src_tstamp_str, src_format, dst_format,
    ...
    """
    将源timeStamp字符串转换为目标timeStamp字符串，
     如果服务器和本地时区都被识别，则尝试应用正确的偏移量，如果不是或者如果tz_offset为假，则根本没有偏移（即假设它们都在同一个TZ中）。
     @param src_tstamp_str：包含timeStamp的STR值。
     @param src_format：解析本地timeStamp时使用的格式。
     @param dst_format：格式化生成的timeStamp时使用的格式。
     @param server_to_client：指定timeZone偏移方向（如果为True则server=src 和 client=dest，反之client=src和
                              server=dest）
     @param ignore_unparsable_time：如果为True，如果无法使用src_format解析src_tstamp_str或使用dst_format格式化，则返回False。
     @return：目标格式化的时间戳，如果可能，如果tz_offset为真，则以目标时区表示;如果无法确定时区偏移，则为src_tstamp_str。
    """
...
class ProductProduct(models.Model):
    ...
    isroom = fields.Boolean('Is Space')  # 这里可能有毒，这个应该是用来判断是否为房间的，而他拿去判断是否已经预约
...
class FolioRoomLine(models.Model):
    ...
    room_id = fields.Many2one('hotel.room', 'Space id')
...
class HotelRoom(models.Model):
    ...
    # 空间类型
    categ_id = fields.Many2one('hotel.room.type', string='Space Category',
                               required=True)
    # 空间设施列表
    room_amenities = fields.Many2many('hotel.room.amenities', 'temp_tab',
                                      'room_amenities', 'rcateg_id',
                                      help='List of space amenities. ')
    # 空间状态
    status = fields.Selection([('available', 'Available'),  # 可用的
    ...
    def check_capacity(self):
        for room in self:
            if room.capacity <= 0:
                raise ValidationError(_('space capacity must be more than 0'))  # 空间容量必须大于0
    ...
    def isroom_change(self):
        ...
        基于isroom，状态将更新
        强势注释：如果被改变为不是房间后，将状态改为Occupied
        ----------------------------------------
...
class HotelFolio(models.Model):
    ...
    # 开始日期
    checkin_date = fields.Datetime('Start Time', required=True, readonly=True,
                                   states={'draft': [('readonly', False)]},
                                   default=_get_checkin_date)
    # 离开日期
    checkout_date = fields.Datetime('End Time', required=True, readonly=True,
                                    states={'draft': [('readonly', False)]},
    ...
    room_lines = fields.One2many('hotel.folio.line', 'folio_id',
                                 ...
                                 states={'draft': [('readonly', False)],
                                         'sent': [('readonly', False)]},
                                 help="Space reservation detail.")
    # 服务行
    service_lines = fields.One2many('hotel.service.line', 'folio_id',
                                    readonly=True,
                                    states={'draft': [('readonly', False)],
                                            'sent': [('readonly', False)]},
                                    help="Space services details provided to"
                                         "Customer and it will included in "
                                         "the main Invoice.")
    # 付款政策
    # On Booking 预订时
    # On Check In 在登记入住hotel_policy
    # On Checkout 在结帐时
    hotel_policy = fields.Selection([('prepaid', 'On Booking'),
                                     ('manual', 'On Check In'),
                                     ('picking', 'On Checkout')],
                                    'Pay Policy', default='prepaid',
                                    help="Site policy for payment that "
    ...
    def folio_room_lines(self):
        ...
        for room in self[0].room_lines:
            if room.product_id.id in folio_rooms:
                raise ValidationError(_('You Cannot Take Same Siee Twice'))
    ...
    def onchange_dates(self):
        ...
        这种方法给出了入住和结账之间的持续时间，如果客户将只考虑留下一整天。
        如果客户将签入结账多于或等于几小时，这在公司中配置为额外的小时数，而不是考虑为全天。
        --------------------------------------------------------------------
        @param self: object pointer
...
class HotelFolioLine(models.Model):
    ...
    folio_id = fields.Many2one('hotel.folio', string='Folio',
                               ondelete='cascade')
    checkin_date = fields.Datetime(string='Start Time', required=True,
                                   default=_get_checkin_date)
    checkout_date = fields.Datetime(string='End Time', required=True,
                                    default=_get_checkout_date)
    ...
    def check_dates(self):
        ...
        if self.checkin_date >= self.checkout_date:
            raise ValidationError(_('Site line Check In Date Should be \
...
class HotelServiceLine(models.Model):
    ...
    _description = 'Space Service line'
...
#     _description = 'Space Services and its charges'
#
...
class ResCompany(models.Model):
    ...
    # 额外小时
    # 提供办理入住时间，结账日期的最小时数值，根据将计算的额外天数，此处提供的任何小时数。
...
class AccountInvoice(models.Model):
    ...
    #     if self._context.get('folio_id'):
    #         print('AccountInvoice write')
    #         print(self._context['folio_id'])
    #         folio = self.env['hotel.folio'].browse(self._context['folio_id'])
            # folio.write({})
